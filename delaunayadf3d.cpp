...
//This function is way too messy, we should rewrite it when we'll have time
void DelaunayADF3D::generate_inner_points() {
    ...
    //The algorithm of zonal division above is rather slow, I am not sure hoe we can make it any faster, but it can bottleneck the rest of the code here.
...
//----------------------------------------------------------
    ...
    while (!insertion_front_.empty()) {
        ...
        for (const auto& c : candidate_list) {
            ...
            //Finding the best node to insert
...
//Basic debug function, delete later
...
//Another generator, much less sophisticated than Delaunay-ADF
void DelaunayADF3D::generate_delaunay_only() {
    ...
    for (const auto& i : inner_edges_) {
        divisions = static_cast<uint32_t>(node_distance(nodes.find(i.node_ids_[0])->second, nodes.find(i.node_ids_[1])->second) /  (1. / 6.));
        if (divisions >= 2) {
    ...
    for (uint32_t i = 0; i < new_nodes.size(); ++i) {
        is_bad = false;
        for (uint32_t j = i + 1; j < new_nodes.size(); ++j)
            if (node_distance(new_nodes[i], new_nodes[j]) < 1. /12.) {
                is_bad = true;
                break;
            }
